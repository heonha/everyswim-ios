...
final class ActivityDatePickerViewController: BaseDatePickerViewcontroller {
    ...
    func selectPicker(to target: Date?, title: String? = "") {
        ...
        if self.viewModel.getDateRange() == .weekly {
            ...
            guard let targetRow = viewModel
                .getRows()[0]
                .firstIndex(of: title) else {return}
        ...
        guard let targetRow = viewModel
            .getRows()[0]
            .firstIndex(of: targetRowString) else {return}
        ...
        if self.viewModel.getDateRange() == .monthly {
            ...
            guard let targetRow = viewModel
                .getRows()[1]
                .firstIndex(of: targetRightRowString) else {return}
    ...
    private func bind() {
        let input = ActivityDatePickerViewModel.Input(applyButtonTap: applyButton.tapPublisher)
...
extension ActivityDatePickerViewController: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return viewModel.getComponents().count
    }
    
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        guard !viewModel.getComponents().isEmpty || !viewModel.getRows().isEmpty  else { return 0 }
        return viewModel.getRows()[component].count
    }
    
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        guard !viewModel.getComponents().isEmpty || !viewModel.getRows().isEmpty else { return "error" }
        return viewModel.getRows()[component][row]
    }
    ...
    private func updateSelectedData() {
        ...
        let left = viewModel.getRows()[componentIndex][leftRow]
        
        if range == .monthly {
            let componentIndex = 1
            let rightRow = datePicker.selectedRow(inComponent: componentIndex)
            right = viewModel.getRows()[componentIndex][rightRow]
        }
        
        guard let selectedDateRange = viewModel.updateSelectedRangesData(selectedDataRange: range, 
                                                                         left: left,
                                                                         right: right) else { return }
